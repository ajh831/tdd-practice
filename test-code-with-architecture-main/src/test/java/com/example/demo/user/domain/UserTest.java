...
public class UserTest {
    ...
    public void User는_UserCreate_객체로_생성할_수_있다() {
        ...
        // then
        assertThat(user.getId()).isNull(); // id가 wrapper 클래스라서 기본값이 Null
    ...
    public void User는_UserUpdate_객체로_데이터를_업데이트_할_수_있다() {
        ...
        // then
        assertThat(user.getId()).isEqualTo(1L); // id가 wrapper 클래스라서 기본값이 Null
        assertThat(user.getEmail()).isEqualTo("kok202@kakao.com");
    ...
    public void User는_로그인을_할_수_있고_로그인시_마지막_로그인_시간이_변경된다() {
        ...
        // when
        user = user.login(new TestClockHolder(1978530673958L));

        // then
        assertThat(user.getLastLoginAt()).isEqualTo(1978530673958L);
    }
